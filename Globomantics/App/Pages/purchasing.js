...
function Purchasing(props) {
    ...
    var handleValueChange = function (name) { return function (event) {
        var _a;
        updateStockRequest(__assign({}, stockRequest, (_a = {}, _a[name] = event.target.value, _a)));
    }; };
    ...
    function lowStockDataSelectionHandler(selection) {
        if (selection.detail.dataPoints.length > 0) {
            ...
            updateStockRequest(__assign({}, stockRequest, { productCode: selectedProduct, type: "increase" }));
        }
        else {
            updateStockRequest(__assign({}, stockRequest, { productCode: "", type: "" }));
        }
    }
    function highStockDataSelectionHandler(selection) {
        if (selection.detail.dataPoints.length > 0) {
            var selectedProduct = DataPointReader.readDataPoint(selection.detail, "Products", "ProductCode");
            updateStockRequest(__assign({}, stockRequest, { productCode: selectedProduct, type: "sell" }));
        }
        else {
            updateStockRequest(__assign({}, stockRequest, { productCode: "", type: "" }));
        }
